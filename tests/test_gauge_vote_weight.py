...
@given(
    st_deposits=strategy('uint256[3]', min_value=10**18, max_value=10**23),
    st_length = strategy('uint256[3]', min_value=2, max_value=52),
    st_votes = strategy('decimal[2][3]', min_value=0, max_value="0.5")
)
...
def test_gauge_weight_vote(accounts, gauge_controller, three_gauges, voting_escrow, st_deposits, st_length, st_votes):
    ...
    votes = []
    for i, acct in enumerate(accounts[:3]):
        votes.append([int(i*10000) for i in st_votes[i]])
    ...
    while history[-1].timestamp < timestamp + max_duration:
        ...
        weights = [gauge_controller.gauge_relative_weight(three_gauges[i]) / 1e18 for j in range(3)]

        theoretical_weights = [
            sum(votes[i][0] * models[i](relative_time) for i in range(3)),
            sum(votes[i][1] * models[i](relative_time) for i in range(3)),
            sum(votes[i][2] * models[i](relative_time) for i in range(3)),
        ]
