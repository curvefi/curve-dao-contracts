...
def test_vote_change(rpc, accounts, gauge_controller, three_gauges):
    gauge_controller.vote_for_gauge_weights(three_gauges[1], 1234, {'from': accounts[0]})
...
def test_vote_remove(rpc, accounts, gauge_controller, three_gauges):
...
def test_vote_expired(accounts, rpc, gauge_controller, three_gauges):
    rpc.sleep(YEAR * 2)
    with brownie.reverts("Your token lock expires too soon"):
        gauge_controller.vote_for_gauge_weights(three_gauges[0], 10000, {'from': accounts[0]})
...
def test_over_weight_adjust_existing(rpc, accounts, gauge_controller, three_gauges):
