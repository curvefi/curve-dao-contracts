...
def test_gauge_integral(accounts, chain, mock_lp_token, token, gauge_v3, minter, gauge_controller):
...
def test_gauge_integral_extra(accounts, chain, mock_lp_token, token, gauge_v3, minter, gauge_controller):
    ...
    # Let Alice and Bob have about the same token amount
    bal = mock_lp_token.balanceOf(alice) // 2
    mock_lp_token.transfer(bob, bal, {"from": alice})
