...
def test_voting_powers(web3, chain, accounts, token, voting_escrow):
    ...
    assert voting_escrow.getPriorVotes(alice, stages["before_deposits"][0]) == 0
    assert voting_escrow.getPriorVotes(bob, stages["before_deposits"][0]) == 0
    assert voting_escrow.totalSupplyAt(stages["before_deposits"][0]) == 0

    w_alice = voting_escrow.getPriorVotes(alice, stages["alice_deposit"][0])
    assert approx(w_alice, amount // MAXTIME * (WEEK - H), TOL)
    assert voting_escrow.getPriorVotes(bob, stages["alice_deposit"][0]) == 0
    w_total = voting_escrow.totalSupplyAt(stages["alice_deposit"][0])
    assert w_alice == w_total

    for i, (block, t) in enumerate(stages["alice_in_0"]):
        w_alice = voting_escrow.getPriorVotes(alice, block)
        w_bob = voting_escrow.getPriorVotes(bob, block)
    ...
    w_alice = voting_escrow.getPriorVotes(alice, stages["alice_withdraw"][0])
    w_bob = voting_escrow.getPriorVotes(bob, stages["alice_withdraw"][0])
    assert w_alice == w_bob == w_total == 0

    w_total = voting_escrow.totalSupplyAt(stages["alice_deposit_2"][0])
    w_alice = voting_escrow.getPriorVotes(alice, stages["alice_deposit_2"][0])
    w_bob = voting_escrow.getPriorVotes(bob, stages["alice_deposit_2"][0])
    assert approx(w_total, amount // MAXTIME * 2 * WEEK, TOL)
    assert w_total == w_alice
    assert w_bob == 0

    w_total = voting_escrow.totalSupplyAt(stages["bob_deposit_2"][0])
    w_alice = voting_escrow.getPriorVotes(alice, stages["bob_deposit_2"][0])
    w_bob = voting_escrow.getPriorVotes(bob, stages["bob_deposit_2"][0])
    assert w_total == w_alice + w_bob
    ...
    w_alice = voting_escrow.getPriorVotes(alice, stages["bob_withdraw_1"][0])
    w_bob = voting_escrow.getPriorVotes(bob, stages["bob_withdraw_1"][0])
    assert w_total == w_alice
    assert approx(w_total, amount // MAXTIME * (WEEK - 2 * H), TOL)
    assert w_bob == 0
