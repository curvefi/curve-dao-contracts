...
def main(deployer=DEPLOYER):
    ...
    # deploy the burners
    underlying_burner = UnderlyingBurner.deploy(
        distributor, RECOVERY, OWNER_ADMIN, EMERGENCY_ADMIN, {"from": deployer}
    )
    MetaBurner.deploy(distributor, RECOVERY, OWNER_ADMIN, EMERGENCY_ADMIN, {"from": deployer})
    usdn_burner = USDNBurner.deploy(
        proxy, distributor, RECOVERY, OWNER_ADMIN, EMERGENCY_ADMIN, {"from": deployer}
    )

    btc_burner = BTCBurner.deploy(
        underlying_burner, RECOVERY, OWNER_ADMIN, EMERGENCY_ADMIN, {"from": deployer}
    )
    CBurner.deploy(underlying_burner, RECOVERY, OWNER_ADMIN, EMERGENCY_ADMIN, {"from": deployer})
    YBurner.deploy(underlying_burner, RECOVERY, OWNER_ADMIN, EMERGENCY_ADMIN, {"from": deployer})
