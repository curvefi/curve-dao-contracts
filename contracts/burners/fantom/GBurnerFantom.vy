...
"""
@title gToken Burner
@notice Converts gToken lending coins to USDT
"""
...
def burn(_coin: address) -> bool:
    ...
    if underlying == USDT:
        LendingPool(LENDING_POOL).withdraw(underlying, amount, self.receiver)
    else:
        registry_swap: address = AddressProvider(ADDRESS_PROVIDER).get_address(2)
        LendingPool(LENDING_POOL).withdraw(underlying, amount, self)
