...
def burn(_coin: address) -> bool:
    ...
    # unwrap aTokens for underlying asset and transfer to receiver
    underlying: address = aToken(_coin).UNDERLYING_ASSET_ADDRESS()
    if underlying == USDC:
        LendingPool(AAVE_LENDING_POOL).withdraw(underlying, amount, self.receiver)
    else:
        registry_swap: address = AddressProvider(ADDRESS_PROVIDER).get_address(2)
        LendingPool(AAVE_LENDING_POOL).withdraw(underlying, amount, self)
        ...
        RegistrySwap(registry_swap).exchange_with_best_rate(underlying, USDC, amount, 0, self.receiver)
