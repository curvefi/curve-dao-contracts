...
def burn_coin(_coin: address):
    Burner(self.burners[_coin]).burn_coin(_coin)
...
def kill_me(_pool: address):
    assert msg.sender == self.emergency_admin
    Curve(_pool).kill_me()
...
def commit_new_parameters(_pool: address,
    ...
    Curve(_pool).commit_new_parameters(amplification, new_fee, new_admin_fee)
...
@public
@nonreentrant('lock')
def revert_new_fee(_pool: address):
    assert msg.sender == self.parameter_admin
    Curve(_pool).revert_new_fee()
...
def set_aave_referral(_pool: address, referral_code: uint256):
    assert msg.sender == self.ownership_admin
    Curve(_pool).set_aave_referral(referral_code)
